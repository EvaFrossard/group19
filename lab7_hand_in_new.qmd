...
pca_fit |> 
  ...
  geom_point(size = 1.5) +
  scale_color_manual(
    values = c(malignant = "pink", benign = "darkblue")
  ) 

```

The PC coordinates are combined with the gravier_clean_aug data set. The categorical variable early_metastasis is colored. On the y-axis the fittedPC variable is 2 and on the x-axis the fitted PC variable is 1. The interpretation of the PC variables is complicated when using categorical variables and only patterns can be recognized. There is no pattern discernible in this scatterplot. Further consideration of the data is necessary.
